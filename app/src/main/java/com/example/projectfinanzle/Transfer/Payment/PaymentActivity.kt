...
class PaymentActivity : AppCompatActivity() {
        ...
        walletRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(this@PaymentActivity, "ไม่มีข้อมูล Wallet", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@PaymentActivity, "เกิดข้อผิดพลาด: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun calBalance() {
        ...
        if (selectedWalletName.isEmpty()) {
            Toast.makeText(this, "กรุณาเลือก Wallet", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        if (walletId == null) {
            Toast.makeText(this, "ไม่พบ Wallet นี้", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        walletRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(accountSnapshot: DataSnapshot) {
                ...
                if (inputAmount == null || inputAmount <= 0) {
                    Toast.makeText(this@PaymentActivity, "กรุณากรอกจำนวนเงินที่ถูกต้อง", Toast.LENGTH_SHORT).show()
                    return
                }
                ...
                walletRef.addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onDataChange(walletSnapshot: DataSnapshot) {
                        ...
                        if (newWalletBalance < 0) {
                            Toast.makeText(this@PaymentActivity, "ยอดเงินคงเหลือไม่พอ", Toast.LENGTH_SHORT).show()
                        } else {
                            recordTransaction(userId, walletId, "expense", inputAmount, descriptionText.text.toString())
                            walletRef.child("wallets").child(walletId).child("balance").setValue(newWalletBalance)
                            Toast.makeText(this@PaymentActivity, "อัปเดตยอดเงินสำเร็จ", Toast.LENGTH_SHORT).show()
                            finish()
                        }
                    }

                    override fun onCancelled(walletError: DatabaseError) {
                        Toast.makeText(this@PaymentActivity, "เกิดข้อผิดพลาด: ${walletError.message}", Toast.LENGTH_SHORT).show()
                    }
                })
            }

            override fun onCancelled(accountError: DatabaseError) {
                Toast.makeText(this@PaymentActivity, "เกิดข้อผิดพลาดในการดึงข้อมูลบัญชี", Toast.LENGTH_SHORT).show()
            }
        })
    }
