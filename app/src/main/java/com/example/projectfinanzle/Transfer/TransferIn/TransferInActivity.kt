...
import com.example.projectfinanzle.databinding.ActivityTransferInBinding
...
class TransferInActivity : AppCompatActivity() {
    private lateinit var binding: ActivityTransferInBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        binding = ActivityTransferInBinding.inflate(layoutInflater)
        ...
        if (user == null) {
            ...
            loadWalletData()
            val uid = user.uid
            database.child(uid).child("balance").addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    balanceUse.text = "Available Balance: ${snapshot.getValue(Double::class.java) ?: 0.0}"
                }
        ...
        walletRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(this@TransferInActivity, "ไม่มีข้อมูล Wallet", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@TransferInActivity, "เกิดข้อผิดพลาด: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun transferInBalance() {
        ...
        if (selectedWalletName.isEmpty() || inputAmount == null || inputAmount <= 0) {
            Toast.makeText(this@TransferInActivity, "กรุณาเลือก Wallet และกรอกจำนวนเงินที่ถูกต้อง", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        accountBalanceRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(accountSnapshot: DataSnapshot) {
                ...
                if (accountBalance < inputAmount) {
                    Toast.makeText(this@TransferInActivity, "ยอดเงินคงเหลือไม่พอ", Toast.LENGTH_SHORT).show()
                    return
                }
                ...
                walletRef.addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onDataChange(walletSnapshot: DataSnapshot) {
                        ...
                        Toast.makeText(this@TransferInActivity, "โอนเงินเข้าวอลเล็ตสำเร็จ", Toast.LENGTH_SHORT).show()
                    }

                    override fun onCancelled(walletError: DatabaseError) {
                        Toast.makeText(this@TransferInActivity, "เกิดข้อผิดพลาด: ${walletError.message}", Toast.LENGTH_SHORT).show()
                    }
                })
            }

            override fun onCancelled(accountError: DatabaseError) {
                Toast.makeText(this@TransferInActivity, "เกิดข้อผิดพลาดในการดึงข้อมูลบัญชี", Toast.LENGTH_SHORT).show()
            }
        })
    }
